...
const checkEmail = createAction(CHECK_EMAIL, (checkEmail,checkColor) => ({ checkEmail,checkColor }));
  ...
  return function (dispatch, getState, { history }) {
    axios({
      ...
      .then((res) => {
        ...
        dispatch(checkEmail("정상적인 이메일입니다.","green"));
        // console.log(res);
        // history.push("/signDog");
        window.alert("정상적인 이메일입니다");
      })
      .catch((err) => {
        dispatch(checkEmail("중복입니다","red"));
